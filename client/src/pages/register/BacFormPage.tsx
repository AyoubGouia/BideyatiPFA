...
export default function BacFormPage({ nav }: Props) {
  ...
  const [section, setSection] = useState<Section>(
    (data.section as Section) || "Math",
  );
  const [session, setSession] = useState<"Principale" | "Contrôle">(
    data.session || "Principale",
  );
  const [region, setRegion] = useState(data.region || "");

  const [notesP, setNotesP] = useState<Record<string, string>>(
    data.notesPrincipale || {},
  );
  const [notesC, setNotesC] = useState<Record<string, string>>(
    data.notesControle || {},
  );
  const [rattrapageChoice, setRattrapageChoice] = useState<string | null>(
    data.selectedRattrapage || null,
  );
  ...
  const mg = useMemo(
    () => calculateMG(section, notesP, notesC, session),
    [section, notesP, notesC, session],
  );
  const fg = useMemo(
    () => calculateFG(section, mg, notesP, notesC, session),
    [section, mg, notesP, notesC, session],
  );
  ...
  const validate = (): boolean => {
    ...
    if (session === "Contrôle") {
      if (sectionChoices && !rattrapageChoice)
        e.rattrapage = "Veuillez choisir une matière.";
      for (const sub of subjects) {
      ...
      <div className={s.scroll}>
        ...
        <div className={s.sectionCard}>
          ...
          <div className={s.inputGroup} style={{ marginTop: "10px" }}>
            ...
            <div className={s.fieldWrap}>
              ...
              {errors.region && (
                <span className={s.errMsg}>{errors.region}</span>
              )}
            </div>
          </div>
        </div>
        ...
        <div className={s.sectionCard}>
          ...
          <p className={s.sectionSub}>
            Saisissez vos notes officielles de la session principale
          </p>
            ...
            <div className={s.row2}>
              {subjects.map((sub) => (
                <div key={sub} className={s.fieldWrap}>
                  ...
                  <input
                    className={`${s.input} ${errors[sub + "_P"] ? s.inputError : ""}`}
                    type="number"
                    step="0.01"
                    value={notesP[sub] || ""}
                    disabled={sub === "Éducation Physique" && exemptSport}
                    onChange={(e) => setNoteP(sub, e.target.value)}
                  />
                  {sub === "Éducation Physique" && (
                    ...
                    >
                      <input
                        ...
                        checked={exemptSport}
                        onChange={(e) => {
                          setExemptSport(e.target.checked);
                          if (e.target.checked) setNoteP(sub, "");
                        }}
                      />{" "}
                      Dispensé(e)
                    </label>
                  ...
                  {errors[sub + "_P"] && (
                    <span className={s.errMsg}>{errors[sub + "_P"]}</span>
                  )}
                </div>
              ))}
            </div>
        ...
        {session === "Contrôle" && (
          <div
            ...
            style={{ border: "2px solid #F47920", background: "#fffcf9" }}
          >
            <p className={s.sectionTitle} style={{ color: "#F47920" }}>
              Notes : Session de Contrôle
            </p>
            <p className={s.sectionSub}>
              Seules les matières rattrapables sont affichées
            </p>

            {sectionChoices && (
              <div style={{ marginBottom: "16px" }}>
                <p className={s.label} style={{ marginBottom: "8px" }}>
                  Matière à rattraper (Choix obligatoire) :
                </p>
                <div className={s.optionGrid}>
                  {sectionChoices.map((c) => (
                    <div
                      key={c}
                      className={`${s.option} ${rattrapageChoice === c ? s.optionSelected : ""}`}
                      onClick={() => setRattrapageChoice(c)}
                    >
                      <div
                        className={`${s.checkbox} ${rattrapageChoice === c ? s.checkboxChecked : ""}`}
                      >
                        {rattrapageChoice === c && (
                ...
                {errors.rattrapage && (
                  <p className={s.errMsg}>{errors.rattrapage}</p>
                )}
              </div>
            )}
            ...
            <div className={s.inputGroup}>
              <div className={s.row2}>
                {subjects
                  .filter((sub) => {
                    if (sub === "Option" || sub === "Éducation Physique")
                      return false;
                    if (sectionChoices) {
                      const otherChoice = sectionChoices.find(
                        (o) => o !== rattrapageChoice,
                      );
                  ...
                  .map((sub) => (
                    <div key={sub} className={s.fieldWrap}>
                      <label className={s.label}>{sub}</label>
                      <input
                        className={`${s.input} ${errors[sub + "_C"] ? s.inputError : ""}`}
                        type="number"
                        step="0.01"
                        placeholder="Note rattrapage"
                        value={notesC[sub] || ""}
                        onChange={(e) => setNoteC(sub, e.target.value)}
                      />
                      {errors[sub + "_C"] && (
                        <span className={s.errMsg}>{errors[sub + "_C"]}</span>
                      )}
                    </div>
                  ))}
              </div>
        ...
        <div className={s.dashboard}>
          <div className={s.dashboardTitle}>
            Résultats{" "}
            {session === "Contrôle" ? "Session Contrôle" : "Session Principale"}
          </div>
          <div className={s.statGrid}>
            <div className={s.statBox}>
              <span className={s.statLabel}>MOYENNE GÉNÉRALE</span>
              <span className={`${s.statValue} ${s.statValueOrange}`}>
                {mg.toFixed(3)}
              </span>
            </div>
            <div className={s.statBox}>
          ...
          <p className={s.dashboardFooter}>
            Calculé selon la formule :{" "}
            {session === "Contrôle" ? "MG_c (Max P/C)" : "MG_principale"}.
          </p>
        </div>

        <div className={s.navRow}>
          <button className={s.btnPrev} onClick={() => nav("register")}>
            Retour
          </button>
          <button
            className={s.btnNext}
