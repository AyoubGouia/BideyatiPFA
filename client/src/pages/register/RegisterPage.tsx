...
export default function RegisterPage({ nav }: Props) {
  ...
  const validate = (): boolean => {
    ...
    if (!form.numeroBAC.trim()) {
      ...
      e.numeroBAC =
        "Le numéro de Bac doit être composé de 6 chiffres exactement.";
    }
    ...
    else if (form.pwd && form.pwd !== form.confirm)
  ...
  return (
    <div className={s.page}>
      ...
      <div className={s.scroll}>
        <BrandHeader
          step="Étape 1 / 3"
          subtitle="Inscription – Informations personnelles"
        />
          ...
          <div className={s.inputGroup}>
            <div className={s.row2}>
              <div className={s.fieldWrap}>
                <label className={s.label} htmlFor="r-nom">
                  Nom *
                </label>
                <input
                  id="r-nom"
              ...
              <div className={s.fieldWrap}>
                <label className={s.label} htmlFor="r-prenom">
                  Prénom *
                </label>
                <input
                  id="r-prenom"
                ...
                />
                {errors.prenom && (
                  <span className={s.errMsg}>{errors.prenom}</span>
                )}
              </div>
            </div>

            <div className={s.fieldWrap}>
              <label className={s.label} htmlFor="r-dob">
                Date de naissance *
              </label>
              <input
                id="r-dob"
            ...
            <div className={s.fieldWrap}>
              <label className={s.label} htmlFor="r-bac">
                Numéro d'inscription au Bac *
              </label>
              <input
                id="r-bac"
                type="text"
                ...
                value={form.numeroBAC}
                onChange={(e) => setField("numeroBAC", e.target.value)}
              />
              {errors.numeroBAC && (
                <span className={s.errMsg}>{errors.numeroBAC}</span>
              )}
            </div>

            <div className={s.fieldWrap}>
              <label className={s.label} htmlFor="r-email">
                Email *
              </label>
              <input
                id="r-email"
            ...
            <div className={s.fieldWrap}>
              <label className={s.label} htmlFor="r-pwd">
                Mot de passe *
              </label>
            ...
            <div className={s.fieldWrap}>
              <label className={s.label} htmlFor="r-conf">
              ...
              {errors.confirm && (
                <span className={s.errMsg}>{errors.confirm}</span>
              )}
            </div>
          </div>
        </div>
        ...
        <div className={s.navRow}>
          <button className={s.btnPrev} onClick={() => nav("home")}>
            Retour
          </button>
          <button
            className={s.btnNext}
