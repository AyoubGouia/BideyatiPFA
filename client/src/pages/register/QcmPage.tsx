...
const QUESTIONS: Question[] = [
  {
    ...
    sub: "Tu es plus attiré(e) par :",
    multi: true,
    options: [
      { icon: "💻", label: "La technologie" },
      { icon: "🔬", label: "Les sciences" },
      { icon: "🤝", label: "Les relations humaines" },
      { icon: "🎨", label: "L'art et la créativité" },
    ],
  },
  {
    id: "q2",
    section: "Matières préférées",
    sub: "Tes matières préférées sont :",
    multi: true,
    options: [
      { icon: "🧮", label: "Mathématiques" },
      { icon: "🔭", label: "Sciences (physique, SVT…)" },
      { icon: "🌐", label: "Langues" },
      { icon: "📊", label: "Économie / gestion" },
    ],
  },
  {
    id: "q3",
    section: "Compétences",
    sub: "Tu te considères plutôt :",
    multi: true,
    options: [
      { icon: "🧩", label: "Logique" },
      { icon: "🎭", label: "Créatif(ve)" },
      { icon: "🤝", label: "Sociable" },
      { icon: "📋", label: "Organisé(e)" },
    ],
  },
  {
    id: "q4",
    section: "Personnalité",
    sub: "Tu préfères plutôt :",
    multi: false,
    options: [
      { icon: "📏", label: "Suivre des règles" },
      { icon: "💡", label: "Innover et tester" },
      { icon: "🫂", label: "Aider les autres" },
      { icon: "👑", label: "Diriger une équipe" },
    ],
  },
  {
    id: "q5",
    section: "Futur professionnel",
    sub: "Tu te vois travailler :",
    multi: false,
    options: [
      { icon: "🏢", label: "Dans un bureau" },
      { icon: "🌳", label: "Sur le terrain" },
      { icon: "🏠", label: "À distance" },
      { icon: "✈️", label: "En déplacement" },
    ],
  },
  {
    id: "q6",
    section: "Motivations",
    sub: "Ce qui te motive le plus :",
    multi: true,
    options: [
      { icon: "💰", label: "Argent" },
      { icon: "❤️", label: "Passion" },
      { icon: "🛡️", label: "Sécurité" },
      { icon: "🏆", label: "Reconnaissance" },
    ],
  },
  {
    id: "q7",
    section: "Questions originales",
    sub: "Ton activité préférée le week-end :",
    multi: false,
    options: [
      { icon: "📺", label: "Regarder des vidéos / séries" },
      { icon: "📚", label: "Lire / apprendre" },
      { icon: "🎨", label: "Créer" },
      { icon: "🎮", label: "Jouer / expérimenter" },
    ],
  },
  {
    id: "q8",
    section: "Questions originales",
    sub: "Ton environnement idéal :",
    multi: false,
    options: [
      { icon: "🔇", label: "Calme et concentré" },
      { icon: "🎵", label: "Animé et dynamique" },
      { icon: "🌿", label: "Nature et extérieur" },
      { icon: "🏙️", label: "Ville et modernité" },
    ],
  },
  {
    id: "q9",
    section: "Questions originales",
    sub: "Ton rôle idéal dans un groupe :",
    multi: false,
    options: [
      { icon: "🎯", label: "Le stratège" },
      { icon: "🔧", label: "L'exécutant" },
      { icon: "💬", label: "Le communicant" },
      { icon: "🌟", label: "Le créatif" },
    ],
  },
];
...
export default function QcmPage({ nav }: Props) {
  ...
  const validate = (): boolean => {
    ...
    QUESTIONS.forEach((q) => {
      if (!answers[q.id] || answers[q.id].size === 0) empty.add(q.id);
    });
    setUnanswered(empty);
    if (empty.size > 0) {
      setGlobalError(
        `Veuillez répondre à toutes les questions (${empty.size} réponse${empty.size > 1 ? "s" : ""} manquante${empty.size > 1 ? "s" : ""}).`,
      );
      return false;
    }
    return true;
  ...
  const handleTerminer = async () => {
    ...
    try {
      ...
      console.log(
        "[QcmPage] Step 1: Registration successful (token saved, state not yet updated)",
      );
      ...
      const allSubjects = Array.from(
        new Set([...Object.keys(notesP), ...Object.keys(notesC)]),
      );
      const formattedNotes = allSubjects
        .map((sub) => {
          ...
          const isNonRattrapable =
            sub === "Option" || sub === "Éducation Physique";
          const effectiveVal =
      ...
      console.log("[QcmPage] Step 2: Submitting questionnaire + notes...", {
        reponses: formattedReponses.length,
  ...
  QUESTIONS.forEach((q) => {
    (grouped[q.section] = grouped[q.section] ?? []).push(q);
  });
  ...
  return (
    <div className={s.page}>
      ...
      <div className={s.scroll}>
        <BrandHeader
          step="Étape 3 / 3"
          subtitle={"Questionnaire d'orientation\nuniversitaire (QCM)"}
        />

        {globalError && <div className={s.globalError}>⚠️ {globalError}</div>}
          ...
          return (
            <div key={secName} className={s.sectionCard}>
              ...
              {qs.map((q, qi) => (
                <div
                  key={q.id}
                  style={{ marginBottom: qi < qs.length - 1 ? 18 : 0 }}
                >
                  <p
                    className={`${s.sectionSub} ${unanswered.has(q.id) ? s.sectionSubError : ""}`}
                  >
                    {q.sub}
                    {unanswered.has(q.id) && (
                      <span className={s.reqBadge}> — Réponse requise</span>
                    )}
                  </p>
                  <div className={s.optionGrid}>
                    {q.options.map((opt) => {
                      ...
                      return (
                        ...
                        >
                          <span
                            className={`${s.checkbox} ${sel ? s.checkboxChecked : ""}`}
                          >
                            {sel && (
      ...
      <p className={s.footer}>© 2026 bideyeti | Tous droits réservés.</p>
    </div>
  );
}
