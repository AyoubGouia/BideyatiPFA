...
import {
  aiApi,
  isAxiosApiError,
  type AiSpecialityOverviewResponse,
} from "../api/aiApi";
...
export default function SpecialiteDetailPage({
  ...
  const [aiPanelState, setAiPanelState] = useState<AiPanelState>({
    status: "idle",
  });
  const [aiReplayKey, setAiReplayKey] = useState(0);
  const aiOverviewCacheRef = useRef<
    Record<string, AiSpecialityOverviewResponse>
  >({});
  ...
  useEffect(() => {
    ...
    if (!user) {
      setAiPanelState({
        status: "auth",
        message:
          "Connectez-vous pour obtenir une lecture IA personnalisee de cette specialite.",
      });
      return;
    }
  ...
  const facultyHistoryForUser = useMemo(() => {
    ...
    if (!userSectionStats.length) {
      if (user && sectionNom !== "Indéfini") {
        console.warn(
          `[AdmissionHistory] No stats found for section: ${sectionNom} (${userSectionKey})`,
        );
      }
      return null;
    }
  ...
  const filiereData = useMemo(() => {
    ...
    const merged = {
      ...
      domaine: base?.domaine || {
        id: "GEN",
        nom: specialite.domaine || "Général",
      },
    ...
    if (specialite.metiers && specialite.metiers.length > 0) {
      specialite.metiers.forEach((m) => {
        ...
        const exists = merged.metiers.some(
          (em) => em.nom.toLowerCase() === metier.titre.toLowerCase(),
        );
        if (!exists) {
          merged.metiers.push({
            nom: metier.titre,
    ...
    if (
      !merged.taux_admission &&
      specialite.statistiquesAdmissions &&
      specialite.statistiquesAdmissions.length > 0
    ) {
      // Find latest valid tauxAdmission
      const latestWithTaux = specialite.statistiquesAdmissions.find(
        (s) => s.tauxAdmission !== null && s.tauxAdmission !== undefined,
      );
      if (latestWithTaux && latestWithTaux.tauxAdmission !== null) {
        ...
        if (t) {
          ...
          if (t < 20)
            merged.taux_admission = `Très sélectif (${Math.round(t)}%)`;
          else if (t < 40)
            merged.taux_admission = `Sélectif (${Math.round(t)}%)`;
    ...
    <div className={s.page}>
      ...
      <main className={s.main}>
        <section className={s.hero}>
          ...
          <p className={s.code}>
            Code orientation : {specialite.codeOrientation}
          </p>
          {specialite.domaine && (
                ...
                <div key={metier.nom} className={s.jobCard}>
                  ...
                  <div className={s.jobSkills}>
                    {metier.competences.map((c) => (
                      <span key={c} className={s.skillChip}>
                        {c}
                      </span>
                    ))}
                  </div>
              ...
              <div className={s.competencesSection}>
                ...
                <div className={s.competencesList}>
                  {filiereData.competences.map((c) => (
                    <span key={c} className={s.competenceChip}>
                      {c}
                    </span>
                  ))}
                </div>
          ...
          <div className={s.controlHeader}>
            <div>
              ...
              <p className={s.cardHint}>
                Alternez entre les donnees d'admission et la lecture IA sans
                quitter la page.
              </p>
            </div>
          ...
          <>
            <section className={s.card}>
              ...
              {statsForYear.length === 0 ? (
                <p className={s.empty}>
                  Aucune statistique d'admission pour cette annee.
                </p>
              ) : (
            ...
            <section className={s.card}>
              <h2 className={s.cardTitle}>
                Capacites par section Bac ({year})
              </h2>
              {sortedCaps.length === 0 ? (
                <p className={s.empty}>
                  Aucune capacite enregistree pour cette annee.
                </p>
              ) : (
                <div className={s.tableWrap}>
                  <table className={s.table}>
            ...
            {user?.studentProfile?.score &&
              ...
              (() => {
                ...
                const fReg = normalize(
                  specialite.etablissement?.gouvernorat ??
                    (specialite.universite as any)?.region,
                );
                ...
                if (user.notes) {
                  user.notes.forEach((n: any) => {
                    noteMap[n.matiereNom] = String(n.valeur);
                  });
                }

                // Use calculateT instead of the base FG score for university-specific scores
                const baseScore = calculateT(
                  user.studentProfile.score,
                  specialite.nom,
                  sectionKey,
                  noteMap,
                );
                const boostedScore = hasBonus
                  ? +(baseScore * 1.07).toFixed(3)
                  : baseScore;
...
function resolveAiErrorState(error: unknown): AiPanelState {
  if (isAxiosApiError(error)) {
    ...
    return {
      status: "error",
      message:
        "L'analyse IA est temporairement indisponible. Reessayez dans quelques instants.",
    };
  }
  ...
  return {
    status: "error",
    message:
      "Une erreur inattendue a interrompu l'analyse IA. Reessayez dans quelques instants.",
  };
}

function resolveTauxLevel(
  taux: string,
): "very-selective" | "selective" | "medium" | "open" {
  const t = taux.toLowerCase();
  if (t.includes("très sélectif") || t.includes("10-20") || t.includes("15%"))
    return "very-selective";
  if (t.includes("sélectif") || t.includes("20-40") || t.includes("30%"))
    return "selective";
  if (t.includes("moyen") || t.includes("40-60") || t.includes("50%"))
    return "medium";
  return "open";
}
