...
function regionDescription(
  ...
  const cityLabel =
    citiesCount > 1
      ? `${citiesCount} villes universitaires`
      : "1 ville universitaire";
...
export default function RegionPage({ nav }: Props) {
  ...
  const [selectedRegion, setSelectedRegion] = useState<RegionGroup | null>(
    null,
  );
      ...
      try {
        ...
        const nextRegions = [...grouped.values()]
          .map((region) => ({
            ...
            faculties: [...region.faculties].sort((a, b) =>
          ...
          <>
            <section className={s.hero}>
              ...
              <p className={s.subtitle}>
                Decouvrez les etablissements reels par localisation geographique
              </p>
            </section>
            ...
            {loading ? (
              ...
              <div className={s.noFaculties}>
                ...
                <p>
                  Aucun etablissement backend ne correspond a votre recherche.
                </p>
              </div>
            ) : (
                  ...
                  >
                    <div className={s.regionHeader}>
                      ...
                      <span className={s.facultyCount}>
                        {region.faculties.length} etablissement
                        {region.faculties.length > 1 ? "s" : ""}
                      </span>
                    </div>
                    ...
                    <div className={s.citiesList}>
                      ...
                      <div className={s.cities}>
                        {region.cities.map((city, index) => (
                          <span key={`${city}-${index}`} className={s.cityTag}>
                            {city}
                          </span>
                        ))}
                      </div>
                ...
                <div className={s.regionStats}>
                  <div className={s.stat}>
                    <span className={s.statNumber}>
                      {selectedRegion.faculties.length}
                    </span>
                    <span className={s.statLabel}>Etablissements</span>
                  </div>
                  <div className={s.stat}>
                    <span className={s.statNumber}>
                      {selectedRegion.cities.length}
                    </span>
            ...
            <section className={s.facultiesSection}>
              ...
              {selectedRegion.faculties.length > 0 ? (
                  ...
                  {selectedRegion.faculties.map((faculty) => (
                    ...
                    >
                      ...
                      <div className={s.facultyPrograms}>
                        <span className={s.programsLabel}>
                          Specialites principales:
                        </span>
                        <div className={s.programsList}>
                          {faculty.programs
                            ...
                            .map((program, index) => (
                              <span
                                key={`${program}-${index}`}
                                className={s.programTag}
                              >
                                {program}
                              </span>
                            ))}
                          {faculty.programs.length > 3 && (
                            <span className={s.morePrograms}>
                              +{faculty.programs.length - 3}
                            </span>
                          )}
                          {faculty.programs.length === 0 && (
                            <span className={s.programTag}>
                ...
                <div className={s.noFaculties}>
                  ...
                  <p>
                    Cette region n'a pas encore d'etablissements disponibles
                    dans le backend.
                  </p>
                </div>
              )}
