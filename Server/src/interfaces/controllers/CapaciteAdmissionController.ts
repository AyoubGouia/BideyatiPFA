...
import { CapaciteAdmissionService } from "../../application/services/CapaciteAdmissionService";

export class CapaciteAdmissionController {
  private service = new CapaciteAdmissionService();
  ...
  getAll = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      console.error(error);
      res.status(500).json({ message: "Erreur lors de la récupération des capacités" });
    }
  };
  ...
  search = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const data = await this.service.search(specialiteId, year, sectionId, tour, skip, take);
      const total = await this.service.count(specialiteId, year, sectionId, tour);
      ...
      console.error(error);
      res.status(500).json({ message: "Erreur lors de la recherche des capacités" });
    }
  };
  ...
  getToursByYear = async (req: Request, res: Response): Promise<void> => {
    try {
      const year = parseInt(req.params.year as string);
      ...
      const tours = await this.service.getToursByYear(year);
